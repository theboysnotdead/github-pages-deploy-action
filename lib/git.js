...
function generateBranch(action, repositoryPath) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield util_1.execute(`git commit --allow-empty -m "Initial ${action.branch} creation"`);
...
function deploy(action) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield util_1.execute(`rm -rf tmp-deployment-folder`);
        yield util_1.execute(`git worktree add --checkout tmp-deployment-folder origin/${action.branch}`);
        yield util_1.execute(`cp -rf ${action.folder}/* tmp-deployment-folder`);
        yield util_1.execute(`cd tmp-deployment-folder`);
        yield util_1.execute(`git add --all .`);
        yield util_1.execute(`git checkout -b new-deploy-changes`);
        yield util_1.execute(`git commit -m "Deploying to ${action.branch} from ${action.baseBranch || 'master'} ${process.env.GITHUB_SHA}"`);
        yield util_1.execute(`git push ${repositoryPath} new-deploy-changes:${action.branch}`);
