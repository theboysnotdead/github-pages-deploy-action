...
function deploy(action) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const status = yield execute(`git status --porcelain`);
        yield execute(`rm-rf !(${action.folder}) && cp -r  ${action.folder}/.  ./ && rm -r ${action.folder}`);
        yield execute(`git add .`);
        yield execute(`git commit -m "Deploying to GitHub Pages"`);
        yield execute(`git push --force ${repositoryPath} ${action.baseBranch || 'master'}:${action.branch}`);
