...
function init() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield exec_1.default.exec('git init');
        yield exec_1.default.exec(`git config user.name ${pusher.name}`);
        yield exec_1.default.exec(`git config user.email ${pusher.email}`);
...
function deploy(action) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const gitStatus = yield exec_1.default.exec(`git status --porcelain`);
        yield exec_1.default.exec(`cd ${action.folder}`);
        if (gitStatus) {
            console.log('There is currently nothing to deploy, aborting...');
            return;
        }
        yield exec_1.default.exec(`git add .`);
        yield exec_1.default.exec(`git commit -m "Deploying to GitHub Pages"`);
        yield exec_1.default.exec(`git push --force ${repositoryPath} ${action.baseBranch ? action.baseBranch : 'master'}:${action.branch}`);
