...
function init() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield child_process_1.exec(`cd ${folder}`);
        yield child_process_1.exec('git init');
        yield child_process_1.exec(`git config user.name ${pusher.name}`);
        yield child_process_1.exec(`git config user.email ${pusher.email}`);
...
function deploy(action) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const gitStatus = yield child_process_1.exec(`git status --porcelain`);
        if (gitStatus) {
            console.log('There is currently nothing to deploy, aborting...');
            return;
        }
        yield child_process_1.exec(`git add .`);
        yield child_process_1.exec(`git commit -m "Deploying to GitHub Pages"`);
        yield child_process_1.exec(`git push --force ${repositoryPath} ${action.baseBranch ? action.baseBranch : 'master'}:${action.branch}`);
